...
export function migrate(d: Database.Database) {
  ...
  // Agent primer toggle. NULL = inherit `default.primer_enabled`; 0 =
  // off; 1 = on. The primer teaches the agent that it's running
  // inside War Room and what HTTP endpoints it can call on the
  ...
  // is unset, ensureSyncClient() short-circuits to "disabled" with no
...
// ─── Agent primer toggle ──────────────────────────────────────────────────
//
// The primer is the War-Room-knowledge MD that teaches the agent how to
// log decisions, post announcements, etc. on the user's behalf. Per-
// channel override + global default, same shape as framework.

export function getChannelPrimerEnabled(channelId: string): boolean | null {
  const row = db()
    .prepare(`SELECT primer_enabled FROM channel_overrides WHERE channel_id = ?`)
    .get(channelId) as { primer_enabled: number | null } | undefined;
  if (row?.primer_enabled === null || row?.primer_enabled === undefined) return null;
  return row.primer_enabled === 1;
}

export function setChannelPrimerEnabled(channelId: string, enabled: boolean | null) {
  const now = Date.now();
  const v = enabled === null ? null : enabled ? 1 : 0;
  db()
    .prepare(
      `INSERT INTO channel_overrides(channel_id, primer_enabled, created_at, updated_at)
       VALUES(?, ?, ?, ?)
       ON CONFLICT(channel_id) DO UPDATE SET
         primer_enabled = excluded.primer_enabled,
         updated_at = excluded.updated_at`,
    )
    .run(channelId, v, now, now);
}
...
export function getAnnouncementById(id: number): AnnouncementRow | undefined {
  return db()
    .prepare(
      `SELECT *, 0 as ack_count, 0 as acked_by_me FROM announcements WHERE id = ?`,
    )
